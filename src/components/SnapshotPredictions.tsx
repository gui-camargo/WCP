...
export default function SnapshotPredictions({
  ...
  const getPointsColor = (points: number | null) => {
    ...
    if (points === 3) return 'text-emerald-700 bg-emerald-100 border-emerald-400'
    if (points === 1) return 'text-orange-700 bg-orange-100 border-orange-400'
  ...
  const correctCount = predictions.filter(p => p.points === 3).length
  const hitRate = predictions.length > 0 ? Math.round((correctCount / predictions.length) * 100) : 0
      ...
      <div className="px-4 py-2">
          ...
          {sortedPredictions.map((pred, idx) => {
            ...
            return (
              <div key={pred.user_id} style={{ display: 'flex', alignItems: 'stretch', gap: '4px', background: 'white', border: '2px solid #e5e7eb', borderRadius: '8px', padding: '4px 8px', boxSizing: 'border-box', minHeight: '28px', overflow: 'visible' }}>
                {isFinished && idx < 3 && (
                  <span style={{ fontSize: '13px', lineHeight: '20px', flexShrink: 0, alignSelf: 'center' }}>
                    {idx === 0 ? '🥇' : idx === 1 ? '🥈' : '🥉'}
                  </span>
                )}
                ...
                {isFinished && pred.points !== null ? (
                  <span className={getPointsColor(pred.points)} style={{ display: 'inline-block', flexShrink: 0, fontWeight: 700, borderRadius: '999px', border: '1px solid currentColor', fontSize: '10px', lineHeight: '20px', padding: '0 6px', alignSelf: 'center', boxSizing: 'border-box' }}>
        ...
        <div className="border-t border-gray-200 mt-2 pt-2">
          <div className="grid grid-cols-2 gap-1">
            <div className="text-center rounded bg-green-50 px-2 py-1 border border-green-200">
            ...
            <div className="text-center rounded bg-orange-50 px-2 py-1 border border-orange-200">
              <p className="text-[10px] text-orange-700 font-semibold">📉 Menos</p>
              <p className="text-sm font-bold text-orange-800">
                {scoreStats.least} <span className="text-[9px] text-orange-600">({scoreStats.leastCount}x)</span>
              </p>
            </div>
            <div className="text-center rounded bg-blue-50 px-2 py-1 border border-blue-200">
            ...
            <div className="text-center rounded bg-emerald-50 px-2 py-1 border border-emerald-200">
              <p className="text-[10px] text-emerald-700 font-semibold">✅ Taxa</p>
              <p className="text-sm font-bold text-emerald-800">
                {hitRate}% <span className="text-[9px] text-emerald-600">acerto</span>
              </p>
            </div>
          </div>
        </div>
