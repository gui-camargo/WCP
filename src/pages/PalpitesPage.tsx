...
export default function ResultadosPage() {
  ...
  const selectedGroupStanding = selectedGroupId
    ? groupStandings.find(g => g.group_id === selectedGroupId) ?? null
    : null
    ...
    if (groupCodes.length > 0) {
      ...
      const normalized: GroupStanding[] = groupCodes
        ...
        .map(code => {
          ...
          return {
            group_id: (row as any)?.group_id ?? `missing-${code}`,
            group: (row as any)?.group ?? { id: '', code },
  ...
  async function loadGroupPreds(groupStanding: GroupStanding) {
    if (!groupStanding.group_id || !canViewAllGroupPreds) return

    setSelectedGroupId(groupStanding.group_id)
    ...
    const [predsRes, leaderboardRes] = await Promise.all([
      supabase
        ...
        .eq('pool_id', poolId!)
        .eq('group_id', groupStanding.group_id),
      supabase
        .from('leaderboard')
        .select('user_id, user_name')
        .eq('pool_id', poolId!),
            ...
            <section key={phaseSection.phase} className="space-y-4">
              ...
              <div className="space-y-5">
                {phaseSection.groups.map(group => (
                  <section key={group.groupCode} className="space-y-2.5">
                    {(() => {
                      ...
                      return (
                        <>
                          <button
                            ...
                            className="w-full flex items-center gap-3 py-0.5"
                            aria-expanded={isOpen}
                          >
                            <span className="h-px flex-1 bg-gradient-to-r from-transparent via-slate-300 to-slate-200" />
                            <span className="inline-flex items-center gap-1.5 text-xs sm:text-sm font-extrabold tracking-wide text-gray-700 uppercase text-center">
                          ...
                          {isOpen && group.rounds.map(round => (
                            <div key={round.roundName} className="space-y-2">
                              <div className="px-1 flex items-center justify-center">
                                <span className="inline-flex items-center rounded-full bg-indigo-100 text-indigo-700 text-[10px] font-bold px-2 py-0.5 border border-indigo-200">
                                  <span className="sm:hidden">{compactRoundLabel(round.roundName)}</span>
                                  <span className="hidden sm:inline">{round.roundName}</span>
                                </span>
                              </div>
                                ...
                                {round.matches.map(m => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={canViewPreds ? 'cursor-pointer hover:scale-[1.01] transition-transform' : 'cursor-default opacity-90'}
                                    >
                                      <ResultMatchCard
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {filteredGroupStandings.map(group => {
                ...
                return (
                  <div key={group.group?.code ?? group.group_id} className="modern-card border border-slate-200 p-4 space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      <h2 className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-slate-50 px-2.5 py-0.5 text-[11px] font-black uppercase tracking-[0.12em] text-slate-700">
                        <span className="inline-block h-1.5 w-1.5 rounded-full bg-brand-500" />
                        Grupo {groupCode}
                      </h2>
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ${canViewAllGroupPreds ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                        {canViewAllGroupPreds ? 'Palpites liberados' : 'Aguardando liberação global'}
                      </span>
                    </div>
                  ...
                  <div className="sticky top-0 z-10 bg-white border-b border-slate-200 px-4 pt-2 pb-3 sm:px-5">
                    ...
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <div className="flex items-center justify-center gap-3">
                        ...
                        />
                        <div className={`text-center rounded px-1.5 py-0.5 text-2xl font-extrabold ${match.status === 'encerrado' ? 'text-black' : 'text-slate-400'}`}>
            ...
            <div className="flex-1 overflow-auto p-4 sm:p-5">
              {loadingPreds ? (
                ...
                (() => {
                  ...
                  const realHomeWins = match.home_score! > match.away_score!
                  const realAwayWins = match.home_score! < match.away_score!
                        ...
                        {sortedPreds.map((p, i) => {
                          ...
                          return (
                            <div key={i} className={`rounded-xl border-2 bg-white px-3 py-2 flex items-center gap-2 ${getPointsBorder(p.points)}`}>
                              <span className="font-semibold text-xs text-gray-800 truncate flex-1 min-w-0">{p.user_name}</span>

                              <div className="flex items-center gap-1 shrink-0">
                                {match.home_team?.flag_code && (
                                  <div className={`rounded p-1 flex items-center justify-center overflow-hidden ${getBannerColor(winnerCorrect)}`}>
                      ...
                      <div className="hidden sm:block rounded-xl overflow-hidden border border-gray-100 bg-white">
                        <table className="w-full text-sm">
                          ...
                          <tbody className="divide-y divide-gray-100">
                            {[...userPreds].sort((a, b) => a.user_name.localeCompare(b.user_name, 'pt-BR')).map((p, i) => {
                              const predictedHomeWins = p.home_guess > p.away_guess
                              const predictedAwayWins = p.home_guess < p.away_guess
                              const predictedDraw = p.home_guess === p.away_guess
                  ...
                  {[...groupPreds].sort((a, b) => a.user_name.localeCompare(b.user_name, 'pt-BR')).map((row, i) => {
                    ...
                    return (
                      <div key={i} className="rounded-xl border border-slate-200 bg-white px-3 py-2 flex items-center justify-between gap-2">
                        <span className="font-semibold text-sm text-gray-800 truncate">{row.user_name}</span>
